<template>
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 justify-items-center w-full relative">
        <Spinner v-if="loading" class="absolute left-0 top-0 bg-white right-0 bottom-0 flex items-center justify-center"/>
            ...
            dark:bg-gray-800 dark:border-gray-700">
                <a href="#" class="border-solid border-b-2 block  m-3 border-slate-200">
                    <img class="object-contain h-48 w-96 md:mb-2 mb-4" :src="product.image_url" alt="product image" />
                </a>
                <div class="px-5 pb-5">
                    <a href="#">
                    ...
                    <div class="flex items-center flex-col justify-between">
                        ...
                        <a href="#" class="text-white bg-amber-700 hover:bg-amber-800 
                        ...
                        rounded-lg text-sm px-4 py-2 text-center hover:translate-y-[-1px] transition-all duration-400">
                            <i class="bi bi-bag-plus-fill mr-2"></i>Add to cart</a>
                    </div>
                </div>
            </div>
    </div>
    ...
    watch(() => route.params.slug, () => {slug.value = route.params.slug, getCategorySingle()});
    onMounted(() => {
        getCategorySingle();
       })
    function getCategorySingle() {
        loading.value = true;
        store.dispatch("getCategorySingle", slug.value).
        then(({data}) => {
            loading.value = false;
